...
docker --version || (notify_webhook "failed" "docker_setup_failed" "ERROR: Docker CLI not working" && exit 1)
docker compose version || (notify_webhook "failed" "docker_setup_failed" "ERROR: Docker Compose not working" && exit 1)
docker buildx version || (notify_webhook "failed" "docker_setup_failed" "ERROR: Docker Buildx not working" && exit 1)

# Ensure current user can access Docker socket
if [ ! -w /var/run/docker.sock ]; then
    notify_webhook "failed" "docker_setup_failed" "Current user $(whoami) cannot access Docker socket, adding to docker group..."
    if ! getent group docker >/dev/null; then
        groupadd docker
    fi
    usermod -aG docker $(whoami)
    notify_webhook "failed" "docker_setup_failed" "You may need to log out/in for group change to take effect"
fi
